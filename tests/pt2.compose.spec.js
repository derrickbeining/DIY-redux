...
describe('compose', () => {
  ...
  xit('composes functions from right to left', () => {
    const composedFunc = compose(square, double, add);
    expect(composedFunc(1, 2, 3)).to.equal(144);
  });

  xit('returns the first function if only one is provided', () => {
    expect(compose(add)).to.equal(add);
  });

  xit('returns the first given argument if no functions are provided', () => {
    expect(compose()(1)).to.equal(1);
    expect(compose()(2, 3)).to.equal(2);
    expect(compose()()).to.equal(undefined);
  });

  xit('will error out if one of the arguments passed in is not a function', () => {
